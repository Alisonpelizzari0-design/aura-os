...
const AuraSync = {
  ...
  async pushAll() {
    ...
    try {
      ...
      // La sauvegarde de l'XP a été supprimée ici
  ...
  async pullAll() {
    ...
    try {
      // On ne charge plus que le profil
      ...
      // La récupération et l'injection des données d'XP ont été supprimées ici
...
/* ─── Modifié pour AURA OS — Gestion intelligente des quotas IA ─── */
...
const AuraSynthese = {
  ...
  async genererHebdo(forcerIA = false) {
    const d      = AuraStore?.get() || {};
   ...
   // Analyse IA si disponible ET demandée explicitement par l'utilisateur
    let iaAnalyse = '';
    const geminiKey = AuraStore?.getGeminiKey?.();
    if (geminiKey && geminiKey !== 'VOTRE_CLE_GEMINI' && forcerIA) {
      try {
...
Génère une synthèse courte (3-4 phrases), encourageante, avec 1 conseil actionnable pour la semaine prochaine.`,
          maxTokens: 300,
          force: true // 🔓 Pass VIP accordé car l'action est déclenchée par un clic utilisateur
        });
      } catch(_) {}
    }
  ...
  async afficherModal(type = 'hebdo', forcerIA = false) {
    // Nettoyer l'ancien modal s'il est déjà ouvert (pour rafraîchir l'écran avec la réponse IA)
    const ancienModal = document.getElementById('synthese-modal');
    if (ancienModal) ancienModal.remove();

    if (forcerIA) {
      AuraUI?.showToast?.('✦ Le Coach AURA analyse tes données...');
    } else {
      AuraUI?.showToast?.('✦ Préparation de ta synthèse...');
    }

    const s = await this.genererHebdo(forcerIA);
    const c = s.contenu;
  ...
  ${[
    ['☀️','Rituels',c.rituels+' complétés','var(--gr-l)','var(--gr)'],
    ['🔥','Streak',c.streak+' jours','#FFF7ED','#C2410C'],
    ['💰','Revenus',_fmt(c.revenus),'var(--gr-l)','var(--gr)'],
    ['💳','Dépenses',_fmt(c.depenses),'var(--re-l)','var(--re)']
  ].map(([ico,lbl,val,bg,col])=>`
    <div style="background:${bg};border-radius:var(--r-md);padding:.75rem;text-align:center">
      ...
      <div style="font-size:14px;font-weight:800;color:${col};margin-top:2px">${val}</div>
          ...
          <div style="background:var(--bg);border:1px dashed var(--bd2);border-radius:var(--r-md);padding:1.25rem;margin-bottom:1.25rem;text-align:center">
            <p style="font-size:12px;color:var(--t2);margin-top:0;margin-bottom:0.75rem">Besoin d'un débrief personnalisé de ta semaine ?</p>
            <button onclick="AuraSynthese.afficherModal('${type}', true)" style="padding:8px 14px;border-radius:var(--r-md);background:var(--pu);color:#fff;border:none;cursor:pointer;font-weight:700;font-size:12px;display:inline-flex;align-items:center;gap:6px">
              ✨ Demander l'analyse au Coach AURA
            </button>
          </div>
          `}
